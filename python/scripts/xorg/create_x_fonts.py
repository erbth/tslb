...
META_PKG_NAME = "xorg-fonts-basic"
...
"""
    ...
    for pkg in ['xorg-libraries', 'mkfontscale']:
    ...
    activated_triggers = []
    if name not in ['font-util', 'encodings']:
        print("  adding fonts-changed trigger")
        activated_triggers.append((name, 'fonts-changed'))

    # Additionally depend on mkfontscale during runtime
    if name.endswith('-type1') or name.endswith('-ttf') or name.endswith('-ethiopic'):
        print("  setting mkfontscale dependency/trigger")

        dl = Constraint.DependencyList()
        dl.add_constraint(Constraint.VersionConstraint(">=", "current"), "mkfontscale")
        cond_update(spv, 'additional_rdeps', [(name, dl)])

        activated_triggers.append((name, 'xorg-fonts-changed'))

        cond_update(spv, 'adapt_command', """#!/bin/bash -e

for DIR in usr/share/fonts/X11/{Type1,TTF,OTF}
do
    find "$DIR" \\( -name "fonts.dir" -o -name "fonts.scale" \\) -exec rm -v {} \; || true
done
""")

    if activated_triggers:
        cond_update(spv, 'activated_triggers_font', activated_triggers)
...
on some basic xorg fonts, following the selection from Beyond Linux® From
Scratch.
EOF
